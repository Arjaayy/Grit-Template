...
export default function TeamsPage() {
  ...
  const filtered = active === "All" ? teams : teams.filter((t) => t.sport === active);
          ...
          <div className="flex flex-wrap gap-3">
            {sports.map((s) => (
              <button
                key={s}
                onClick={() => setActive(s)}
                className={`text-xs font-medium uppercase tracking-widest px-4 py-2 rounded-sm border transition-all duration-200 ${
                  active === s
                    ? "bg-[#DA1D3A] border-[#DA1D3A] text-white"
                    : "border-white/10 text-white/40 hover:border-[#DA1D3A]/40 hover:text-white"
                }`}
              >
                {s}
              </button>
        ...
        <div className="max-w-6xl mx-auto">
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <p className="font-display text-4xl font-black uppercase text-white/10 mb-3">No teams found</p>
              <p className="text-white/30 text-sm">Try selecting a different sport</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
              {filtered.map(({ initials, name, sport, wins, losses, captain, image }) => (
